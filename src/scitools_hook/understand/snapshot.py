...
from scitools_hook.config.models import LINE_METRIC, STATEMENT_METRIC, Settings, ThresholdSpec
...
def _element_metrics(specs: Iterable[ThresholdSpec]) -> dict[Scope, list[str]]:
    ...
    Two routine metrics are asked for whatever the thresholds say, because two readers judge
    a routine by a count off its record and neither reader is a threshold: the net delta sums
    ``CountStmt`` and ``CountLineCode`` on every check that has a before side, whatever
    ``[lean]`` says (lean-code requirement 7.1, follow-up 14), and the pass-through rule reads
    the first (requirement 2.2, follow-up 11). Until those follow-ups both counts reached the
    record only because the shipped thresholds asked for them; a configuration that dropped
    the thresholds had the delta sum zeros and the rule judge nothing, without a word either
    way. A configuration that keeps them produces exactly the request it always did. What
    this does **not** buy is a sentence in the report for a build that lacks a count: the
    worker records such a metric under ``snapshot.unavailable`` as it does any requested one,
    but ``analysis.thresholds._seed_unavailable`` keeps the run's own list to thresholded
    metrics, the delta counts an unmeasured side as zero (``net._count``) and the rule
    declines an unmeasured record without a note (``layering._within_budget``).
    """
    ...
    # Unconditional: the delta runs on every check with a before side regardless of settings,
    # so the request may not depend on a threshold or a `[lean]` switch happening to ask.
    found.setdefault("routine", set()).update((STATEMENT_METRIC, LINE_METRIC))
