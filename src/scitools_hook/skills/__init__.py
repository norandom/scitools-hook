...
deliberately last -- change the rules or tune duplication and dead code detection. The ordering
is the point: ``scitools-gate`` and ``scitools-improve`` both refuse to touch the configuration,
because an agent that can silence its own findings has no gate, and ``scitools-adapt`` is where
that decision is made with evidence instead. ``scitools-tune`` tunes and calibrates the
lean-code rules and safety floors. ``scitools-onboard`` is the one-time act of deciding what a
repository is, and it derives the answer from measurement rather than from these defaults.
