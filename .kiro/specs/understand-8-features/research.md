...
- **Findings**:
  ...
  - Cost, re-measured 2026-09-06 while implementing task 6.1 on Build 1262 over 300 routines of this repository: three `CountGlobals*` **2.056 ms per routine** against **0.027 ms** for three built-ins, a factor of 76; `CountClassCoupledModified` **0.058 ms per class**. Original probe: three `CountGlobals*` on 300 routines 0.67 s (2.2 ms each) against 0.01 s for three built-ins; `CountClassCoupledModified` on 100 classes 0.00 s; `BidirectionalDepsPercent` answered `None` for every file here; `CorePercentage` answered `'0,19'` -- a locale-formatted string the worker already coerces.
