...
- 6.4 MEASURED `und` HAZARD (binds 6.5, 8.2): **`und` prints an error for an unrecognised command and STILL EXITS 0.** `und version` -> `(Build 1204)` rc 0; `und -version` and `und --version` -> `Error: No valid command found` rc 0. So rc 0 alone does NOT mean success — a typo in an argv builder, or a switch this build rejects, looks healthy. Also: `und version` carries only the build, no product version (the API ping returns `6.5.1204`), so nothing may expect a `6.5.x` string from the CLI.
